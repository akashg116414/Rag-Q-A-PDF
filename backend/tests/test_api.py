...
def test_upload_pdf(mocker, mock_db):
    mocker.patch("app.utils.pdf_utils.extract_text_from_pdf", return_value="Extracted PDF text")
...
def test_ask_question(mocker, mock_db, mock_openai):
    mock_retriever = mocker.patch("app.api.routes.qa.FAISS.as_retriever", return_value=MagicMock())
    mock_retriever.return_value.get_relevant_documents.return_value = [{"page_content": "Mocked Answer"}]

    mock_faiss = mocker.patch("app.api.routes.qa.FAISS.from_texts", return_value=MagicMock())
    
    mock_faiss.return_value = MagicMock()
    mock_faiss.return_value.save_local = MagicMock()
    
    mock_embeddings = mock_openai.return_value.embed_documents.return_value = ["mocked_embedding"]
    
    mock_qa_chain = mocker.patch("app.api.routes.qa.RetrievalQA.from_llm", return_value=MagicMock())
